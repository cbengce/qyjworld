...
export function AscendResult({ locale }: { locale: string }) {
  ...
  return <main className="overflow-hidden bg-paper text-forest">
    <section className="px-5 py-14 md:px-8 md:py-20"><div className="mx-auto grid max-w-6xl items-center gap-12 lg:grid-cols-[0.9fr_1.1fr]">
      ...
      <div className="mt-9 flex flex-wrap gap-3"><button className="focus-ring min-h-12 rounded-full bg-forest px-7 font-bold text-white disabled:opacity-50" disabled={generating} onClick={generate} type="button">{generating ? "Creating My Card…" : "Create My Card"}</button><Link className="focus-ring inline-flex min-h-12 items-center rounded-full border border-forest/20 px-7 font-bold" href={`/${locale}/ascend`} onClick={() => trackAscendEvent("ascend_try_again", { locale, profile: profile.slug })}>Try Again</Link><Link className="focus-ring inline-flex min-h-12 items-center rounded-full border border-forest/20 px-7 font-bold" href={`/${locale}/menu`} onClick={() => trackAscendEvent("ascend_menu_clicked", { locale, profile: profile.slug })}>Explore the Menu</Link></div>{referralProgress ? <section aria-labelledby="ascend-journey-heading" className="mt-8 border-l-2 border-gold pl-5"><h2 className="text-xs font-bold uppercase tracking-[0.18em] text-gold" id="ascend-journey-heading">Your Ascend Journey</h2><p className="mt-3 text-sm text-forest/65">Completed journeys through your link:<br /><strong className="text-lg text-forest">{referralProgress.completedTests}</strong></p>{referralProgress.nextMilestone ? <p className="mt-3 text-sm text-forest/65">Next milestone:<br /><strong className="text-forest">{referralProgress.nextMilestone.remainingCount} more to unlock {referralProgress.nextMilestone.rewardName}</strong></p> : null}</section> : null}</div>
    </div></section>
