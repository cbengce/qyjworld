-- NEVER DEPLOYED. Superseded by canonical migration 0030_store_menu_cms.sql.
begin;
...
select r.id, p.id
from public.roles r
join public.permissions p on p.permission_code in ('store.identity.manage', 'store.operations.manage')
...
select r.id, p.id
from public.roles r
join public.permissions p on p.permission_code = 'store.operations.manage'
where r.role_code = 'manager'
on conflict (role_id, permission_id) do update set status = 'active', deleted_at = null;

insert into public.ordering_domain_allowlist (brand_id, hostname)
select id, 'order.qyjworld.com'
from public.brands
where brand_code = 'QYJ'
on conflict (brand_id, hostname) do update set status = 'active', deleted_at = null;
