...
declare
  ...
  v_gross numeric(14,2) := greatest(coalesce((p_event->>'grossAmount')::numeric, 0), 0);
  -- Refund events carry the cumulative valid cash refund for the transaction.
...
begin
  ...
  if v_event_type = 'payment_succeeded' then
    ...
    if v_event_type = 'order_voided' or v_is_full_refund then
      -- The final refund removes all rounding residue without exceeding the original commission.
      v_reversal := v_remaining_commission;
    elsif v_refund_delta > 0 then
      -- Partial refunds target the cumulative proportional reversal, then post only its delta.
